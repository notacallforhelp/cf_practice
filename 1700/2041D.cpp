...
{   
   ...
   vector<vector<vector<vector<int>>>> dist(
        n, vector<vector<vector<int>>>(m, vector<vector<int>> (4,vector<int>(4,MAX))));
   ...
   while(!q.empty())
   {
        auto [x,y,dir,consec] = q.front();
        q.pop();

        if(x==tgX&&y==tgY)
        {
            cout << dist[x][y][dir][consec] << "\n"; return;
        }

        for(int ndir=0;ndir<4;ndir++)
        {
            int nconsec = (ndir==dir)?(consec+1):1;
            if(nconsec>3) continue;
            ...
            if(nx>=0&&nx<n&&ny>=0&&ny<m&&grid[nx][ny]!='#')
            {
                if(dist[nx][ny][ndir][nconsec]==MAX)
                {
                    dist[nx][ny][ndir][nconsec]=dist[x][y][dir][consec]+1;
                    q.push({nx,ny,ndir,nconsec});
                }
            }
        }
   }

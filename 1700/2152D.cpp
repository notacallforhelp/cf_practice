...
{   
   ...
   vector<int> typeA(n+1);
   vector<int> typeB(n+1);
   vector<int> typeC(n+1);

   for(int i=1;i<=n;i++)
   {
        int mx = get_first_bit(A[i]);
        int ct = get_bit_count(A[i]);
   ...
   {
        int l,r; cin>>l>>r;
        int output = typeA[r]-typeA[l-1] + (typeB[r]-typeB[l-1])/2 + typeC[r]-typeC[l-1];

...
{  
    ...
    vector<int> event[N+2];

    for(int i=0;i<n;i++)
    {
        event[L[i]].push_back(i);
        event[R[i]+1].push_back(i);
        event[n-R[i]].push_back(i);
        event[n-L[i]+1].push_back(i);
    }

    vector<int> in_A(n),in_B(n);
    ...
    for(int i=1;i<=n-1;i++)
    {
        ...
        {
            --cnt[in_A[p]][in_B[p]];
            in_A[p] = (i>=L[p]&&i<=R[p]?1:0);
            in_B[p] = ((n-i)>=L[p]&&(n-i)<=R[p]?1:0);
            ++cnt[in_A[p]][in_B[p]];
        }

        bool is_valid = true;

        if(cnt[0][0]>0||cnt[1][0]>i||cnt[0][1]>(n-i)) is_valid = false;

        if(is_valid)
        {
            output = (output + C(cnt[1][1],i-cnt[1][0]))%mod;
        }
    }

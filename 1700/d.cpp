...
{  
    ...
    for(int i=1;i<=h;i++)
    {
        for(int j=1;j<=w;j++)
        {
            ...
            if(grid[i][j]=='G') tgX = i, tgY = j;
        }
    }

    vector<vector<vector<bool>>> vis(h+1, vector<vector<bool>>(w+1, vector<bool>(4)));
    ...
    auto dfs = [&](auto && self, int i,int j,int previ,int prevj) -> void {
        ...
        if(i!=previ||j!=prevj) output.push_back(m[dir]);
        ...
        {
            if(dir==0) self(self,i+1,j,i,j);
            if(dir==1) self(self,i-1,j,i,j);
            if(dir==2) self(self,i,j+1,i,j);
            if(dir==3) self(self,i,j-1,i,j);
        }
        else if(grid[i][j]=='x')
        {
            for(int d=0;d<4;d++)
            {
                if(d==dir) continue;
                if(d==0) self(self,i+1,j,i,j);
                if(d==1) self(self,i-1,j,i,j);
                if(d==2) self(self,i,j+1,i,j);
                if(d==3) self(self,i,j-1,i,j);
            }
        }

...
{   
   int n,m; cin>>n>>m;
   vector<vector<char>> grid(n+2,vector<char>(m+1));
   ...
   //int sz = n*m/2 + 5;
   vector<vector<int>> cc(n+1,vector<int> (m+1,-1));
   vector<int> top(n+2);
   vector<int> bottom(n+2);
   vector<int> left(m+2);
   vector<int> right(m+2);

   auto dfs = [&](auto && self,int i,int j,int c)->void{
        if(i<=0||i>=n+1||j<=0||j>=m+1) return;
        if(cc[i][j]!=-1||grid[i][j]!='#') return;
        cc[i][j] = c;
        self(self,i-1,j,c);
        self(self,i+1,j,c);
        self(self,i,j+1,c);
   ...
   for(int i=1;i<=n;i++)
   {
        output = max(output,m+top[i-1]+bottom[i+1]);
   }

   for(int j=1;j<=m;j++)
   {
        output = max(output,n+left[j-1]+right[j+1]);
   }

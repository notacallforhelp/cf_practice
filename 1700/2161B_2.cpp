...
{   
   ...
   for(auto &ele:grid) cin>>ele;

   int minx = 1e5; int maxx = -1;
   int miny = 1e5; int maxy = -1;

   int maxanti = -1, minanti = 1e5;
   int maxmain = -1e5, minmain = 1e5;
   ...
   for(int i=0;i<n;i++)
   {
        for(int j=0;j<n;j++)
        {
            if(grid[i][j]=='#')
            {
                ...
                maxx = max(j,maxx);
                maxy = max(i,maxy);
                maxanti = max(maxanti,i+j);
                minanti = min(minanti,i+j);
                maxmain = max(maxmain,j-i);
                minmain = min(minmain,j-i);
            }
        }
   }

   if(minx==1e5||(maxx-minx<=1&&maxy-miny<=1)||maxanti-minanti<=1||maxmain-minmain<=1)
   {
        cout << "YES\n";
   }
   else

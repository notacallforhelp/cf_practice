...
{  
    ...
    for(int i=1;i<=n;i++)
    {
        pf[i] = max(pf[i-1],A[i]+(n-i));
        sf[n-i+1] = max(sf[n-i+2],A[n-i+1]+(n-i));
    }
    ...
    for(int i=1;i<=n;i++)
    {
        if(i==0)
        {
            output = min(output,max(A[i],sf[i+1]));
        }
        else if(i==n)
        {
            output = min(output,max(A[i],pf[i-1]));
        }
        else
        {
            output = min(output,max({A[i],pf[i-1],sf[i+1]}));
        }
    }

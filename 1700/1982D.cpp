...
{   
   ...
   vector<vector<char>> chgrid(n+1,vector<char>(m+1));
   for(int i=1;i<=n;i++)
   {
        for(int j=1;j<=m;j++)
        {
   ...
   vector<vector<int>> pfw(n+1,vector<int>(m+1));
   vector<vector<int>> pfwo(n+1,vector<int>(m+1));
   ...
   for(int i=1;i<=n;i++)
   {
        for(int j=1;j<=m;j++)
        {
            pfw[i][j] = pfw[i-1][j] + pfw[i][j-1] - pfw[i-1][j-1];
            pfwo[i][j] = pfwo[i-1][j] + pfwo[i][j-1] - pfwo[i-1][j-1];
            if(chgrid[i][j]=='0')
            {
                w += grid[i][j];
                ++pfw[i][j];
   ...
   for(int i=k;i<=n;i++)
   {
        for(int j=k;j<=m;j++)
        {
            int wgrid = pfw[i][j] - pfw[i-k][j] - pfw[i][j-k] + pfw[i-k][j-k];
            int wogrid = pfwo[i][j] - pfwo[i-k][j] -pfwo[i][j-k] + pfwo[i-k][j-k];

            int v = abs(wgrid-wogrid);

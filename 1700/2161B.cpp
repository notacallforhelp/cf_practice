...
bool func(int x,int y,int _x,int _y,int d1,int d2,int need,vector<string> &grid)
{
    int n = grid[0].size();
    int ct1 = 0;
    int p = x; int q = y;
...
{   
   ...
   for(int i=0;i<n;i++)
   {
        for(int j=0;j<n;j++)
        {
            ...
            if(i+1<n&&j+1<n&&grid[i][j]=='#'&&grid[i+1][j]=='#'&&grid[i][j+1]=='#'&&grid[i+1][j+1]=='#')
            {
                edge = true;
            }

            if(i-2>=0&&grid[i][j]=='#'&&grid[i-1][j]=='#'&&grid[i-2][j]=='#')
            {
                fail = true;
            }
            if(j-2>=0&&grid[i][j]=='#'&&grid[i][j-1]=='#'&&grid[i][j-2]=='#')
            {
                fail = true;
            }
        }
   ...
   ans |= func(x,y,x+1,y,1,-1,ct,grid);
   ans |= func(x,y,x,y-1,1,-1,ct,grid);
   ans |= func(x,y,x,y+1,1,1,ct,grid);
   ans |= func(x,y,x+1,y,1,1,ct,grid);

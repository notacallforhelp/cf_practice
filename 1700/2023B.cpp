...
{   
   ...
   set<pair<int,int>> s;  // {penalty, node}
   s.insert({0,1}); 

   while(!s.empty())
   {
        int node = s.begin()->second;
        int curr_penalty = s.begin()->first;

        s.erase(s.begin());

        if(curr_penalty>d[node]) continue;

        int v_tojump = B[node];
        int cost_jump = A[node];

        if(cost_jump+d[node]<d[v_tojump])
        {
            if(s.count({d[v_tojump],v_tojump}))
            {
                s.erase({d[v_tojump],v_tojump});
            }
            d[v_tojump] = cost_jump + d[node];
            s.insert({d[v_tojump],v_tojump});
        }
        ...
        if(node>1)
        {
            if(d[node-1]>d[node])
            {
                if(s.count({d[node-1],node-1})) s.erase({d[node-1],node-1});
                d[node-1]=d[node];
                s.insert({d[node-1],node-1});
            }
        }

...
struct SegmentTree
{
    ...
    int merge(int left,int right,int lev){
        if(lev%2!=0)
        {
            return (left|right);
        }
    ...
    int build(vector<int> &a,int v,int tl,int tr){
        if(tl==tr){
            t[v]=a[tl];
            return 1;
        }
        ...
        {
            int tm = tl + (tr-tl)/2;
            int left_lev = build(a,2*v,tl,tm);
            int right_lev = build(a,2*v+1,tm+1,tr);

            t[v] = merge(t[2*v],t[2*v+1],left_lev);
    ...
    int update(int v,int tl,int tr,int pos,int val){
        ...
        else
        {
            ...
            int level = 0;
            if(pos<=tm){
                level = update(2*v,tl,tm,pos,val);
            }
            else
            {
                level = update(2*v+1,tm+1,tr,pos,val);
            }

            t[v] = merge(t[2*v],t[2*v+1],level);
            return level+1;
        }
    }
    ...
    int update(int pos,int val){
        return update(1,0,n-1,pos-1,val);
    }
};
...
{  
    int n,m; cin>>n>>m;
    vector<int> A(1<<n); for(auto &ele:A) cin>>ele;
    SegmentTree st(A);

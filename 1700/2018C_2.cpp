...
{   
   ...
   vector<int> bad_children(n+1,0);

   auto revdfs = [&](auto &&self,int v)->void{
        ...
        if(p==0) return;
        bad_children[p]++;
        int total_children = adj[p].size() - (p==1?0:1);
        if(bad_children[p]==total_children)
        {
            self(self,p);
        }
   };

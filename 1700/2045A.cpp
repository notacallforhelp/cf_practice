...
//consosnants - others
// NG can be a single consonant
// syllable = conso + vowel + conso
// word = 1 or more syll
// delete and rearrange to get a word
...
{   
   ...
   int max_len = 0;
   int n = S.length();
   int ct_NG = min(ct_G,ct_N);
   int consonants = n - pure_vowels - ct_Y;

   for(int i= 0;i<=ct_NG;i++)
   {
        int bound1 = pure_vowels + ct_Y;
        int bound2 = (n-i)/3;
        int bound3 = (consonants+ct_Y-i)/2;

        int s = min({bound1,bound2,bound3});

        max_len = max(max_len,3*s + min(2*s,i));
   }

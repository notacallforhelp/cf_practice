...
{   
   ...
   for(int i=1;i<=n;i++) pfA[i]=pfA[i-1]+A[i];
   for(int i=1;i<=m;i++) pfB[i]=pfB[i-1]+B[i];
   vector<pair<int,pair<int,int>>> combined(m+n+1);
   vector<int> pfcombined(m+n+1);
   int p1 = 0;int p2 = 0;
   for(int i=m+n;i>=1;i--)
   {
        if(A[n-p1]>B[m-p2]||m-p2==0)
        {
            combined[i].first = A[n-p1];
            ++p1;
        }
        else if(A[n-p1]<B[m-p2]||n-p1==0)
        {
            combined[i].first = B[m-p2];
            ++p2;
        }
        else if(p1>p2)
        {
        ...
        combined[i].second = {p1,p2};
   }

   for(int i=1;i<=n+m;i++)
   {
        pfcombined[i] = pfcombined[i-1]+combined[i].first;
   }
   ...
   while(q--)
   {
        ...
        int xused = combined[n+m-z+1].second.first;
        int yused = combined[n+m-z+1].second.second;

        if(xused<=x&&yused<=y)
        {
            cout << pfcombined[n+m]-pfcombined[n+m-z] << "\n";
        }
        else if(xused>x)
        {
            int takex = x;
            ...
            cout << pfA[n]-pfA[n-takex] + pfB[m]-pfB[m-takey] << "\n";

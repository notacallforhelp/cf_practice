...
int func(int idx,ld prevpos,vector<int> &A){
    int n = A.size();
    if(idx>=n) return 0;

    ld leftpos = prevpos + A[idx];
    ld rightpos = prevpos - A[idx];

    int p = max(func(idx+1,leftpos,A)+(signbit(leftpos)!=signbit(prevpos))
                ,func(idx+1,rightpos,A)+(signbit(rightpos)!=signbit(prevpos)));

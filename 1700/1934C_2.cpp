...
{  
    ...
    int s1 = query(1,1) + 2;
    int s2 = n+m-query(n,m);
    int s3 = query(1,m)-m+1;

    auto clamp = [&](int val,int limit){
        return max(1ll,min(val,limit));  
    };

    int x1 = clamp((s1+s3)/2,n);
    int y1 = clamp((s1-s3)/2,m);

    int x2 = clamp((s2+s3)/2,n);
    int y2 = clamp((s2-s3)/2,m);

    if(query(x1,y1))
    {
        cout << "! " << x2 << " " << y2 << endl; return;
    }

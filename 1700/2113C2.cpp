...
{   
   ...
   for(int i=1;i<=n;i++)
   {
        ...
        {
            cin>>grid[i][j];
            pf[i][j] = pf[i-1][j] + pf[i][j-1]-pf[i-1][j-1]+(grid[i][j]=='g');
        }
   }
   ...
   for(int i=1;i<=n;i++)
   {
        for(int j=1;j<=m;j++)
        {
            if(grid[i][j]=='.')
            {
                int torem = pf[min(i+k-1,n)][min(j+k-1,m)]
                            -pf[min(i+k-1,n)][max(j-k,0ll)]
                            -pf[max(i-k,0ll)][min(j+k-1,m)]
                            +pf[max(i-k,0ll)][max(j-k,0ll)];
                
                output = max(output,pf[n][m]-torem);
            }
        }
   }

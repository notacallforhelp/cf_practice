...
{   
   ...
   int mn_r = 1e9; int mn_c = 1e9;
   int mx_r = -1; int mx_c = -1;
   int ct = 0;

   auto dfs = [&](auto && self,int i,int j)->void{
        if(i<1||i>n||j<1||j>m||grid[i][j]=='.'||vis[i][j]) return;
        vis[i][j]=1;
        ++ct;
        mn_r = min(mn_r,i); mn_c = min(mn_c,j);
        mx_r = max(mx_r,i); mx_c = max(mx_c,j);
        self(self,i,j-1);
        self(self,i,j+1);
        self(self,i-1,j);
        self(self,i+1,j);
   ...
   {
        SR[i] += SR[i-1];
        output = max(output,SR[i]+FR[i]);
   }

   for(int i=1;i<=m;i++)
   {
        SC[i] += SC[i-1];
        output = max(output,SC[i]+FC[i]);
   }

...
const int T = 2000;  // constant for coord compression, p = x*T + y;
const int R = 2e5; // 4* 50k?

int Ord[R+5]; // stores best positions from first to last
int Nxt(int p,int o) // allows to get all 4 cells in a group depdning on bitmask o
{
    int x = p/T;
    int y = p%T;
    if(o&1) y = y/3*3 +3 -y%3;  // y/3*3 finds base, and then +3-y%3 flips y
    if(o&2) x = x/3*3 +3 -x%3; // same for x
    return x*T+y;
}

bool G[2][T*T];  // G[0] for loners, G[1] for non loners, denotes seat occupance
int Ans[2]; // Ans[0] best pointer for loners, [1] best ptr for non loners

void solve()
{   
   int q; cin>>q;
   for(int i=0;i<q*4;i++) G[0][Ord[i]]=G[1][Ord[i]]=0; //*4 seat clearance
   Ans[0]=Ans[1]=0; // reset ptr
   while(q--)
   {
        int t; cin>>t;

        while(G[t][Ord[Ans[t]]]) Ans[t]++; //finds best seat for type t

        int seat = Ord[Ans[t]];
        cout << seat/T << " " << seat%T << "\n";

        G[1][seat]=1; // marks occupance
...
int32_t main()
{
    ...
    for(int i=2,tp=0;i<T&&tp<R;i++) //i is the diagonal sum
    {
        if(i%3==2)  // top left cell
        {
            for(int x=1;x<i&&tp<R;x+=3)
            {
                Ord[tp++]=x*T+i-x;
            }
        }
        ...
        {
            for(int x=1;x<=i-2&&tp<R;x++)
            {
                if(x%3==1) // top right cell
                {
                    Ord[tp++]=x*T+i-x;
                }
                else if(x%3==2)
                {
                    Ord[tp++]=x*T+i-x-2; //prev bottom right cell first
                    Ord[tp++]=x*T+i-x; // then curr bottom left cell
                }
            }
            Ord[tp++]=(i-1)*T+1; //last bottom left cell manually insert
        }
    }

...
{  
    ...
    auto dfs = [&](auto && self,int p,int v)-> void {
        if(!found) temp.push_back(v);
        if(v==b)
        {
            found = true;
            return;
    ...
    int output = temp.size()/2;

    auto dfs_depth = [&](auto &&self,int p,int v)->int {
        int max_d = 0;
        for(auto &ele:adj[v])
        {
            if(ele!=p)
            {
                max_d = max(max_d,1 + self(self,v,ele));
            }
        }
        return max_d;
    };

    int max_depth = dfs_depth(dfs_depth,0,mid);

    output = output + 2*(n-1) - max_depth;

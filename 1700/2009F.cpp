...
//    }
   ...
   while(q--)
   {
        ...
        --l; --r;
        //cout << l/n << " " << r/n << "\n";
        int totalsets = max(0ll, r/n -1 - (l/n +1)+1);
        //cout << totalsets << endl;
        int output = totalsets*pf[n-1];
        //cout << output << endl;
        if(l/n==r/n)
        {
            int add = l/n;
            ...
            int right = (l/n+1)*n-1;
            int leftidx = (l+add)%n;
            int adder = min(right-left,r-l);
            int rhtifx = min(leftidx+adder,n-1);

            int extra = leftidx + adder - rhtifx;

            //cout << leftidx << " " << rhtifx << " " << adder << " " << extra << endl;;


            output += pf[rhtifx] - (leftidx>0?pf[leftidx-1]:0);
            if(extra>=1)
            {
                output += pf[extra-1];
            }
        ...
        else
        {
            ...
            // int b = ((l/n+1)*n-1)%n;
            // int leftend = min(a,b);
            // int rightend = max(a,b);
            // output += pf[rightend]- (leftend>0?pf[leftend-1]:0);
            // for(int i=l;i<(l/n+1)*n;i++)
            // {
            //     cout << (i+add)%n << endl;
            //     output += A[(i+add)%n];
            // }
            ...
            int rht = (l/n+1)*n -1;
            int lefidx = (l+add)%n;
            int rhtifx = min(lefidx+rht-lef,n-1);

            int loutput = 0;

            int xtra = lefidx + rht -lef - rhtifx;
            //cout << l << " " << add << endl;
            // cout << lef << " " <<rht << endl;
            // cout << lefidx << " " << rhtifx << " " << lefidx + rhtifx - lef << endl;
            // cout << xtra << endl;

            output += pf[rhtifx] - (lefidx>0?pf[lefidx-1]:0);
            if(xtra>=1)
            {
                output += pf[xtra-1];
            }
            ...
            int left = (r/n)*n;
            int right = r;
            int leftidx = (left+add)%n;
            int rightidx = min(leftidx+right-left,n-1);

            // cout << leftidx << " " << rightidx << " " << leftidx+right-left << endl;
            int extra = leftidx + right-left - rightidx;
            // cout << extra << endl;

            output += pf[rightidx]-(leftidx>0?pf[leftidx-1]:0);

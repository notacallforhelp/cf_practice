...
void solve()
{
    int n; cin>>n;
    vector<int> B(2*n);
    for(auto &ele:B)cin>>ele; sort(B.begin(),B.end());
    reverse(B.begin(),B.end());

    vector<int> biggest, smallest;
    ...
    {
        x += B[i];
        biggest.push_back(B[i]);
    }

    for(int i=n+1;i<2*n;i++)
    {
        x -= B[i];
        smallest.push_back(B[i]);
    }

    for(int i=0;i<n-1;i++)
    {
        cout << biggest[i] << " "<< smallest[i] << " ";
    }

    cout << biggest[n-1] << " " << x << " " << biggest[n] << endl;
}

...
{
    int n; cin>>n;
    int r1,c1,r2,c2,r3,c3; cin>>r1>>c1>>r2>>c2>>r3>>c3;
    int x,y; cin>>x>>y;
    ...
    if(M[r1]>1) commonrow=r1;
    if(M[r2]>1) commonrow=r2;
    if(M2[c1]>1) commoncol = c1;
    if(M2[c2]>1) commoncol=c2;

    if((commonrow==1||commonrow==n)&&(commoncol==1||commoncol==n))
    {
        if(commonrow==x||commoncol==y)
        {
   ...
   if((commoncol+commonrow)%2==(x+y)%2)
   {
        if(commonrow%2==x%2||commoncol%2==y%2)
        {
            cout << "YES" << endl; return;
        }
        else

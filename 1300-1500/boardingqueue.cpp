...
void solve()
{
    ...
    while(turns<p)
    {
        ...
        for(int i=0;i<4;i++)
        {
            if(in(x+dx[i],y+dy[i],r,c))
            {
                if(G[x+dx[i]][y+dy[i]]==currp-1)
                {
                    nextx=x+dx[i];
                    nexty=y+dy[i];
                }
                if(G[x+dx[i]][y+dy[i]]>0&&seen.count(G[x+dx[i]][y+dy[i]]+inc)==0&&G[x+dx[i]][y+dy[i]]+inc<=n)
                {
                    seen.insert(G[x+dx[i]][y+dy[i]]+inc);
                }
            }
        }
        x=nextx;

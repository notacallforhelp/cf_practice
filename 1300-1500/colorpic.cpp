...
bool areToroidalNeighbours(int x1, int y1, int x2, int y2, int n, int m) {
    // Vertical neighbours (wraps across rows)
    bool vertical = (y1 == y2) && (
        ( (x1 - x2 + n) % n == 1 ) ||   // +1 mod n
        ( (x1 - x2 + n) % n == n - 1 )  // -1 mod n
    );

    // Horizontal neighbours (wraps across columns)
    bool horizontal = (x1 == x2) && (
        ( (y1 - y2 + m) % m == 1 ) ||   // +1 mod m
        ( (y1 - y2 + m) % m == m - 1 )  // -1 mod m
    );
...
void solve()
{
    ...
    {
        cout << "neighbours of "<< prs[i].first << " " << prs[i].second << endl;
        for(int j=0;j<prs.size();j++)
        {
            if(areToroidalNeighbours(prs[i].first,prs[i].second,prs[j].first,prs[j].second,n,m))
            {
                cout << prs[j].first << " " << prs[j].second << endl;
            }
        }

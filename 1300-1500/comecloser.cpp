...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        s1.insert({prs[i].first,prs[i].second});
        s2.insert({prs[i].second,prs[i].first});
    }
    ...
    for(int i=0;i<n;i++)
    {
        s1.erase({prs[i].first,prs[i].second});
        s2.erase({prs[i].second,prs[i].first});
        int len = (s1.rbegin()->first-s1.begin()->first + 1ll);
        int breadth = (s2.rbegin()->first-s2.begin()->first + 1ll);
        if(len*breadth==(n-1))
        {
            if((len+1)*breadth<=(breadth+1)*len)
            {
                ++len;
            }
            else

...
void solve()
{
    ...
    int ans = max(cntL,cntR) + vis;  // in the spirit of all -1 or -2 along with passengers

    for(int i=1;i<=m;i++)
    {
        if(seats[i])
        {
            ans = max(ans,min(cntL,curL)+min(cntR,curR)+vis); //trying to find the midpoint
        }
        else
        {
            ++curL;

...
int dfs(int vertex,int pos,int answer)
{
    if(vis[vertex])
    {
        return answer;
    ...
    {
        if(M[{vertex,child}]<pos)
        {
            output = max(output,dfs(child,M[{vertex,child}],answer+1));
        }
        else
        {
            output = max(output,dfs(child,M[{vertex,child}],answer));
        }
    }

...
int func(vector<int> &A,int idx,int h,int g,int b)
{
    if(idx==A.size()) return 0;

    if(h>A[idx]) return func(A,idx+1,h+(A[idx]/2),g,b)+1;

    int ans1 = (g?func(A,idx,h*2,g-1,b):0);
    int ans2 = (b?func(A,idx,h*3,g,b-1):0);
...
{
    int n,h; cin>>n>>h;
    vector<int> A(n); for(auto &ele:A)cin>>ele; sort(A.begin(),A.end());

    cout << func(A,0,h,2,1) << endl;
}

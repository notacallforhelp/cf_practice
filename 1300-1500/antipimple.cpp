...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        if(uniq.empty()||uniq.back()!=A[i].first)
        {
            uniq.push_back(A[i].first);
        }
    }
    ...
    auto compute_g = [&](int blocked_ct){
        int m = n-blocked_ct;
        return minus_self(pw2[m],1);
    };
    ...
    {
        int val = uniq[i];
        while(ptr<n&&A[ptr].first>val)
        {
            add_idx(A[ptr].second);
            ++ptr;
        }
        int g_v = compute_g(c);
        ...
        int g_vminus1 = compute_g(c);

        int f = minus_self(g_v,g_vminus1);
        add_self(ans,f%mod*val%mod);
    }

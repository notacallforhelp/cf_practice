...
void solve()
{
    ...
    vector<int> R(n); for(auto &i:R)cin>>i;
    map<int,int> M;

    for(int i=0;i<n;i++)
    {
        for(int j=X[i]-R[i],push=0;j<=X[i];j++,push++)
        {
            int p = R[i]*R[i]-(j-X[i])*(j-X[i]);
            int low = 0;
            int high = R[i];
            int op = -1;
            while(low<=high)
            ...
            M[j]=max(op,M[j]);
            int otherx = (X[i]-j)+X[i];
            //cout << otherx << " " << push << endl;
            M[otherx]=max(op,M[otherx]);
        }
    }
    //cout << endl;

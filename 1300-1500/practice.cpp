...
{
    int n; cin>>n;
    vector<int> heights(n); for(auto &ele:heights)cin>>ele;
    ...
    do
    {
        ...
        for(int i=0;i<heights.size();i++)
        {
            ...
            if(i-1>=0&&heights[i]>heights[i-1]) mn = min(mn,heights[i]);
            if(i+1<heights.size()&&heights[i]>heights[i+1]) mn = min(mn,heights[i]);
        }
        if(mn!=1e9+10)
        {
            heights.erase(find(heights.begin(),heights.end(),mn));
            highlightOrder.push_back(mn);
        }
    }while(mn!=1e9+10);

    for(auto &ele:highlightOrder) cout << ele << " ";
    cout << endl;
}

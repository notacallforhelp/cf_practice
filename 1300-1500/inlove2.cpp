...
void solve()
{
    ...
    while(q--){
        ...
        if(c=='+')
        {       
            start.insert(a); end.insert(b);
        }
        else
        {
            start.erase(start.lower_bound(a)); end.erase(end.lower_bound(b));
        }
        ...
        int minright = *end.begin();
        int maxleft = *start.rbegin();

        if(minright<maxleft) cout << "YES" << endl;
        if(minright>=maxleft) cout << "NO" << endl;
    }
}

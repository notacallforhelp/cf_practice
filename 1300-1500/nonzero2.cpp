...
void solve()
{
    ...
    for(int i=0;i<pos.size();i+=2)
    {
        ...
        for(int j=cur;j<x;j++)
        {
            output.push_back({j,j});
        }

        if(A[x]==A[y])
        {
            if((y-x)%2==1) //diff parity, but same ele so cancels out
            {
                output.push_back({x,y});
            }
            else  // same parity, but parity being manip so cancels out
            {
                output.push_back({x,x});
                output.push_back({x+1,y});
            }
        }
        else
        {
            if((y-x)%2==0) //diff ele, but same parity so cancels out
            {
                output.push_back({x,y});
            }
            else  //diff ele, and diff parity but being manip to same parity
            {
                output.push_back({x,x});
                output.push_back({x+1,y});
            }

...
bool feasible(int k,int n,vector<int>& A,vector<int> &B)
{
    if(k==0) return true;
    ...
    for(int i=0;i<n;i++)
    {
        int lval = A[i]-k+1;
        int rval = A[i]+k-1;
        auto itL = lower_bound(B.begin(),B.end(),lval);
        auto itR = upper_bound(B.begin(),B.end(),rval);
        int l = itL-B.begin();
        int r = (itR-B.begin())-1;
        if(l>r) continue;

        int start = (l+i)%n; if(start<0) start+=n;
        int end = (r+i)%n; if(end<0) end+=n;
        if(start<=end)
        {
            diff[start]++;
            diff[end+1]--;

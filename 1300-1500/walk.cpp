...
void solve()
{
    ...
    {
        cin>>ele;
        if(ele==1) onethere=true;
    }

    int pre_output = (onethere?m:m+1);
    ...
    for(int i=1;i<sz;i++)
    {
        pre_output += max(((shops[i]-1)-shops[i-1])/d ,0ll);
    }
    ...
    for(int i=1;i<sz-1;i++)
    {
        ...
        int thisp = shops[i];
        bool isone=false;
        if(shops[i]==1) isone=true;

        int delta = max(((rp-1)-lp)/d,0ll) -1 -max(((thisp-1)-lp)/d,0ll) - max(((rp-1)-thisp)/d,0ll) + (isone?1:0);

        //ops.insert(delta+pre_output);
        mn_ans = min(mn_ans,delta+pre_output);
        ++ct[delta+pre_output];
    }

    cout << mn_ans << " " << ct[mn_ans] << endl;

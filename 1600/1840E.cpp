...
{   
    ...
    int t, q; cin>>t>>q;
    set<int> st;
    deque<pair<int,int>> prs;
    int time = 1;
    int sz = s.length();
    for(int i=0;i<sz;i++)
    {
        if(s[i]!=p[i]) st.insert(i);
    }
    while(q--)
    {
        while(!prs.empty()&&time>=prs[0].first)
        {
            if(s[prs[0].second]!=p[prs[0].second]) st.insert(prs[0].second);
            prs.pop_front();
        }
        int type; cin>>type;
        if(type==1)
        {
            ...
            prs.push_back({time+t,pos});
        }
        else if(type==2)
        {
            int a,p1,b,p2; cin>>a>>p1>>b>>p2; --p1; --p2;
            if(a==1&&b==1)
            {
                swap(s[p1],s[p2]);
            }

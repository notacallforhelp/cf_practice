...
{   
    ...
    for(int i=0;i<n;i++)
    {
        if(deg[i]!=mx_deg) continue;

        s.erase(s.lower_bound(deg[i]));

        for(auto &ele:adj[i])
        {
            s.insert(deg[ele]-1);
            s.erase(s.lower_bound(deg[ele]));
        }

        output = max(output,mx_deg+*s.rbegin()-1);

        for(auto &ele:adj[i])
        {
            s.insert(deg[ele]);
            s.erase(s.lower_bound(deg[ele]-1));
        }
        s.insert(deg[i]);
    }

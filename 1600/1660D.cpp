...
{   
   ...
   vector<int> minuspf(n+2);
   vector<int> ct2pf(n+2);
   vector<int> zeroes;
   ...
   {
        cin>>A[i];
        minuspf[i] += minuspf[i-1] + (A[i]<0);
        ct2pf[i] += ct2pf[i-1] + (abs(A[i])==2);
   ...
   {
        int start = zeroes[i]+1;
        int end = zeroes[i+1]-1;

        if(start>end) continue;

        int ctminus = minuspf[end]-minuspf[start-1];
        if(ctminus%2==0)
        {
            int ct2 = ct2pf[end]-ct2pf[start-1];
            if(ct2>mx)
            {
                mx = ct2;
                x = start-1;
        ...
        else
        {
            ...
            while(low<=high)
            {
                ...
                if(minuspf[mid]-minuspf[start-1]<=ctminus-1)
                {
                    idx = max(idx,mid);
                    low=mid+1;
                }
            ...
            if(minuspf[idx]-minuspf[start-1]==ctminus-1)
            {
                int ct2 = ct2pf[idx]-ct2pf[start-1];
                if(ct2>mx)
                {
                    mx = ct2;
                    x = start-1;

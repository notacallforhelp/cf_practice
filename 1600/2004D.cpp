...
int func2(string &t, string& b,int x,int y,vector<int> &search)
{
    int cost = 1e9;
    auto it = lower_bound(search.begin(),search.end(),x);
    if(it!=search.end()) cost = min(cost,abs(*it-x)+abs(y-*it));
    if(it!=search.begin())
    {
        auto it2 = prev(it);
        cost = min(cost,abs(*it2-x)+abs(y-*it2));
    }
    return cost;
}
...
{   
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(colors[i]=="BG") BG.push_back(i);
        if(colors[i]=="BR") BR.push_back(i);
        if(colors[i]=="BY") BY.push_back(i);
        if(colors[i]=="GR") GR.push_back(i);
        if(colors[i]=="GY") GY.push_back(i);
        if(colors[i]=="RY") RY.push_back(i);   
    }
    ...
    while(q--)
    {
        ...
        if((a[0]!=b[0])&&(a[0]!=b[1])&&(a[1]!=b[0])&&(a[1]!=b[1]))
        {
            for(auto &c:a)
            {
                ...
                {
                    string t = "";
                    t.push_back(c); t.push_back(d); sort(t.begin(),t.end());
                    if(t=="BG") cost = min(cost,func2(t,b,x,y,BG));
                    if(t=="BR") cost = min(cost,func2(t,b,x,y,BR));
                    if(t=="BY") cost = min(cost,func2(t,b,x,y,BY));
                    if(t=="GR") cost = min(cost,func2(t,b,x,y,GR));
                    if(t=="GY") cost = min(cost,func2(t,b,x,y,GY));
                    if(t=="RY") cost = min(cost,func2(t,b,x,y,RY));
                }
            }
        }
        else
        {
            cost = min(cost,abs(x-y));
        }

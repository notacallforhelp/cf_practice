...
{   
    int n,m; cin>>n>>m;
    vector<int> bound = {n,m};  //n 0s, m 1s
    vector<vector<int>> A(2,vector<int>(n+m+1,0));
    for(int i=0;i<2;i++)
    {
    ...
    {
        int currtype = 0;
        if(A[1][i]>A[0][i]) currtype = 1;

        if(cur[currtype]==bound[currtype])
        {
            currtype = 1-currtype;
            if(bad==-1)
            {
    ...
    {
        int g;
        if(bad!=-1&&i<bad&&type[i]==badtype)
        {
            g = ans -A[type[i]][i] - A[1-badtype][bad] + A[badtype][bad] + A[1-badtype][n+m];
        }
        else
        {
            g = ans - A[type[i]][i] + A[type[i]][n+m];
        }
        result.push_back(g);
    }

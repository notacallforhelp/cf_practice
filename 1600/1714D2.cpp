...
void solve()
{   
   string t; cin>>t; int len = t.length();
   int n; cin>>n;
   vector<string> words(n);
   for(auto &ele:words) cin>>ele;

   vector<int> correct (len);
   vector<int> correct2(len);
   ...
   for(int i=0;i<len;i++)
   {
        ...
        maxconquered=max(maxconquered,upd);

        if(bestidx!=-1)
        {
            output.push_back({bestidx,i});
        }
   }
...
//    cout << endl;
   ...
   for(int i=len-1;i>=0;i--)
   {
        ...
        if(start!=len) output.push_back({bestidx,start});
   }

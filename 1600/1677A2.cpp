...
struct FenwickTree {
    ...
    FenwickTree(vector<int> const&a) : FenwickTree(a.size()) {   // O(n)
        for(int i=0;i<n;i++)
        {
            bit[i] += a[i];
            int r = i|(i+1);
...
{   
     ...
     for(int i=0;i+1<n;i++)
     {
         ...
         int loc = P[i+1]-1-f1.sum(P[i+1]-1);
         for(int j=i+2;j<n;j++)
         {
             int smaller = f1.sum(P[j]-1);
             int greater = f2.sum(P[j]+1,n);
             loc -= greater;
             if(P[j]>P[i])
             {
                 output += loc;

...
void solve()
{   
   vector<vector<int>> pf(1001,vector<int>(1001,0));
   ...
   for(int i=1;i<=1000;i++)
   {
        ...
        {
            pf[i][j]=pf[i-1][j]+pf[i][j-1]-pf[i-1][j-1]+(grid[i][j])*i*j;
        }
   }
    ...
    // cout << endl;
        ...
        while(q--)
        {
                ...
                cout << max(pf[hb-1][wb-1]-pf[hs][wb-1]-pf[hb-1][ws]+pf[hs][ws],0ll) << endl;
        }
}

...
{   
   ...
   {
        if(div*26<n) continue;
        int done = 0;
        int ops = 0;
        deque<char> inc;
        set<char> final;
        for(int i=0;i<min(n/div,26ll);i++)
        {
            if(M[alpha[i]]>=div)
            {
                ++done;
            ...
            {
                ops += div-M[alpha[i]];
                inc.push_back(alpha[i]);
            }
            final.insert(alpha[i]);
        }

        string temp = s;
        map<char,int> tmpct = M;

        for(int i=0;i<n;i++)
        {
            if(tmpct[temp[i]]>div||final.count(temp[i])==0)
            {
                --tmpct[temp[i]];
                temp[i]=inc[0];
                ++tmpct[temp[i]];
                if(tmpct[temp[i]]==div) inc.pop_front();
            }
        }

        ans.push_back({ops,temp});

...
{   
   ...
   {
        int mn = abs(A[0]-1);
        output = min(output,cur+abs(A[0]-x)+abs(x-1));

        for(int i=1;i<n;i++)
        {
            output = min(output,cur+mn-abs(A[i]-A[i-1])+abs(A[i]-x)+abs(A[i-1]-x));
            output = min(output,cur-abs(A[i]-A[i-1])+abs(A[i-1]-1)+abs(x-1)+abs(A[i]-x));
            mn = min(mn,-abs(A[i]-A[i-1])+abs(1-A[i])+abs(1-A[i-1]));
        }

        output = min(output,cur+mn+abs(A[n-1]-x));
